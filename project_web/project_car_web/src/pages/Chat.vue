  ...
  export default {
    data() {
      return {
        ws: null,
        count: 0,
        userId: this.$cookies.get("userId"), // 当前用户ID
        username: this.$cookies.get("username"), // 当前用户昵称
        avatar: this.$cookies.get("avatar"), // 当前用户头像
        list: [], // 聊天记录的数组
    ...
    methods: {
      ...
      initWebSocket() {
        ...
        if (window.WebSocket) {
          ...
          var url = 'ws://' + serverHot + ':8891' + '/groupChat/' + sip + '/' + this.userId; // `ws://127.0.0.1/9101/groupChat/10086/聊天室`
